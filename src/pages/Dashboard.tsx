...
import { ArrowUpRight, Users, CreditCard, BarChart3, Clock } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const platformRevenue = platforms.map(platform => {
      ...
      const totalRevenue = platformSubs.reduce((sum, sub) => sum + sub.cost, 0);
      return {
        name: platform.name,
        value: totalRevenue,
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthlyTotal = monthSubs.reduce((sum, sub) => sum + sub.cost, 0);
  ...
  const totalRevenue = subscriptions.reduce((sum, sub) => sum + sub.cost, 0);
